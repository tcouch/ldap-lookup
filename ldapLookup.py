...
def saveToFile(resultsList):
        ...
        outputKeys = ['Search Term',
                      ...
                      'Faculty',
                      'Full Name']
        writer = csv.DictWriter(open(fname, 'w'), outputKeys,
                                restval="Not Found")
        writer.writeheader()
...
def setupLDAPConnection():
        ...
        ou = user_name[0:4]
        bind_user = "cn=%s,ou=%s,ou=Departments,dc=uclusers," \
                    "dc=ucl,dc=ac,dc=uk" % (user_name, ou)
...
def getResults(con, searchList):
        ...
        for searchTerm in searchList:
                ...
                searchResult = [ldapData for ldapData in ldapHistory
                                if ldapData[searchTermType] == searchTerm]
                if not searchResult:
                        searchResult = queryLDAP(con, searchTerm,
                                                 searchTermType)
                        if searchResult["Username"] != "Not found":
                                ldapHistory.append(searchResult)
                                with open('ldaphistory.pkl', 'wb') \
                                as ldapHistoryFile:
                                        pickle.dump(ldapHistory,
                                                    ldapHistoryFile)
...
def displayOutput(searchList, results):
        ...
        for result in results:
                ...
                print(result["Title"], result["First Name"],
                      result["Last Name"])
